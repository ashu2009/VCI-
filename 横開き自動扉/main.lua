...
function DoorPut()
    ...
    if DOOR_MOVE.open_flg then
        --スケール0基準
        clock_est = (os.clock() - DOOR_MOVE.now_open_clock) / DOOR_MOVE.open_clock_max
        --最大値行かないように
        if clock_est >= 1 then
            clock_est = 1
        end
        ...
        --スケール1基準
        clock_est = (DOOR_MOVE.close_clock_max - (os.clock() - DOOR_MOVE.now_close_clock)) / DOOR_MOVE.close_clock_max
        --最大値行かないように
        if clock_est <= 0 then
            clock_est = 0
        end
    ...
    --相対移動量計算
    local door_move_amount = DOOR_DATA.door_def_moverange * clock_est * flame_si_scale.x
    --ローカル移動量計算
    local loc_move_amount = RelativeToLocalCoordinates(flame_si, door_move_amount, 0, 0)
