...
function onUse(use)
    if count % 4 == 1 then
        --vci.message.Emit("comment", "cdbhi")
        --vci.message.Emit("comment", "あああああああい")
    elseif count % 4 == 2 then
        --vci.message.Emit("comment", "あああああああああああああああい")
        --vci.message.Emit("comment", "cdbhialaebilvnbaebvdbvuyawkvhvcdbhialaebilvnbaebvdbvuyawkvhv")
    elseif count % 4 == 3 then
        --vci.message.Emit("comment", "cdbhialaeb")
        --vci.message.Emit("comment", "ｖわいｌｗい")
    elseif count % 4 == 0 then
    --vci.message.Emit("comment", "vw間をいあｍ：ｖｐばｗ")
    --vci.message.Emit("comment", "e")
    end
    --count = count + 1
...
function ConvexStrShow()
    if ASSET.IsMine then
        ...
        for i = 1, #aveter_data do
            ...
            if put_char + len > (1 + ((i - 1) % 2)) * COMENT_VIEWER.text_str_max then
                if COMENT_VIEWER.save_convex_str ~= "　　　　　　  凸者一覧\n" then
